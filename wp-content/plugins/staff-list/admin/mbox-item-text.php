...
function abcfsl_mbox_item_text_line_bldr( $tplateOptns, $itemOptns, $F ){
    ...
    switch ($fieldType){
        case 'T':
            ...
            break;
        case 'PT':
            abcfsl_mbox_item_text_PT( $tplateOptns, $itemOptns, $F );
            break;
        case 'H':
...
//Paragraph Text.
function abcfsl_mbox_item_text_PT( $tplateOptns, $itemOptns, $F ){

    $lineTxt = isset( $itemOptns['_txt_' . $F] ) ? esc_attr( $itemOptns['_txt_' . $F][0] ) : '';
    $inputLbl = abcfsl_mbox_item_text_line_number( $F , isset( $tplateOptns['_inputLbl_' . $F] ) ? esc_attr( $tplateOptns['_inputLbl_' . $F][0] ) : '' );

    echo abcfl_input_txtarea('txt_' . $F, '', $lineTxt, $inputLbl, '', '100%', '10', '', '', '', 'abcflFldCntr', 'abcflFldLbl  abcflFontW700');
}
