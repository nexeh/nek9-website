 ...
 */
class nek9sar_Services_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_services_widget', // Base ID
			__( 'nek9sar Services Widget', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays pages as services. Best for Business Template Top/Bottom Area.', 'nek9sar' ), ) // Args
		);	
	}
	...
	?>
		<?php _e( 'If there is not an icon it will display the featured image of the page as the icon.<br /> Give the name of the font awesome icon to the Icon input field.You can find the icon names from the readme.txt <br /> eg: fa-angle-double-up', 'nek9sar' ); ?>
		<?php for( $i=0; $i<6; $i++) { ?>
		<?php $j = $i+1; ?>
			<p>
				<label for="<?php echo $this->get_field_id( 'icon'.$i ); ?>"><?php _e( 'Icon ', 'nek9sar' ); echo $j; ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'page_id'.$i ); ?>"><?php _e( 'Page ', 'nek9sar' ); echo $j; ?>:</label>
		...
		<?php while ($get_services_pages->have_posts()) : $get_services_pages->the_post(); ?>
			...
			<div class="<?php echo $class; ?>">
				<div class="th-services-box">
					...
					<div class="th-morelink-sep"><a class="th-morelink" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>"><?php _e( 'Read More', 'nek9sar' ) ?></a></div>
				</div><!-- .nek9sar-services-boxset -->
			</div>
...
function nek9sar_register_services_widget() {
	register_widget( 'nek9sar_Services_Widget' );
}
add_action( 'widgets_init', 'nek9sar_register_services_widget' );
...
class nek9sar_RecentWork_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_recentwork_widget', // Base ID
			__( 'nek9sar Recent Work Widget', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays pages as recent work.Featured image of the page will show up first. Best for Business Template Top/Bottom Area.', 'nek9sar' ), ) // Args
		);	
	}
	...
	?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'nek9sar' ); ?></label> 
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_page_titles' ); ?>"><?php _e( 'Display page titles.', 'nek9sar' ); ?></label> 
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_page_excerpts' ); ?>"><?php _e( 'Display page excerpts.', 'nek9sar' ); ?></label> 
		</p>
		
		<?php for( $i=0; $i<4; $i++) { ?>
			<p>
				<label for="<?php echo $this->get_field_id( key($defaults) ); ?>"><?php _e( 'Page', 'nek9sar' ); ?>:</label>
...
function nek9sar_register_recentwork_widget() {
	register_widget( 'nek9sar_RecentWork_Widget' );
}
add_action( 'widgets_init', 'nek9sar_register_recentwork_widget');


/**
 * Widget to display call to action.
 */
class nek9sar_CallTo_Action extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_callto_action', // Base ID
			__( 'nek9sar Call To Action', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays a call to action widget.Best for Business Template Top/Bottom Area.', 'nek9sar' ), ) // Args
		);	
	}

	public function form( $instance ) {
		$nek9sar_defaults[ 'action_title' ] = '';
		$nek9sar_defaults[ 'action_text' ] = ''; 
		$nek9sar_defaults[ 'button_text' ] = '';
		$nek9sar_defaults[ 'button_url' ] = '';
		$instance = wp_parse_args( (array) $instance, $nek9sar_defaults );
	...
	?>
		...
		<p><?php _e( 'Call to Action Main Text','nek9sar' ); ?></p>
		...
		<p><?php _e( 'Call to Action Additional Text','nek9sar' ); ?></p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('button_text'); ?>"><?php _e( 'Button Text:', 'nek9sar' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('button_url'); ?>"><?php _e( 'Button Redirect Link:', 'nek9sar' ); ?></label> 
...
function nek9sar_register_callto_action_widget() {
	register_widget( 'nek9sar_CallTo_Action' );
}
add_action( 'widgets_init', 'nek9sar_register_callto_action_widget' );
 ...
 */
class nek9sar_SinglePage_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_singlepage_widget', // Base ID
			__( 'nek9sar Single Page Widget', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays a single Page along with the featured image.', 'nek9sar' ), ) // Args
		);	
	}
	...
	<p><?php _e( 'Displays the page title if this title field is empty', 'nek9sar' ); ?></p>

	<p>
		<label for="<?php echo $this->get_field_id( 'page_id' ); ?>"><?php _e( 'Page', 'nek9sar' ); ?></label>
		<?php wp_dropdown_pages( array( 'name' => $this->get_field_name( 'page_id' ), 'selected' => $instance['page_id'] ) ); ?>
	</p>
	<p>
		<input class="checkbox" type="checkbox" <?php echo $disable_feature_image; ?> id="<?php echo $this->get_field_id('disable_feature_image'); ?>" name="<?php echo $this->get_field_name('disable_feature_image'); ?>" /> <label for="<?php echo $this->get_field_id('disable_feature_image'); ?>"><?php _e( 'Remove Featured image', 'nek9sar' ); ?></label>
	</p>
	...
	public function widget( $args, $instance ) {
 		...
 		if( $page_id ) :
	 		...
	 		<div class="th-singlepage-widget">
				...
				<article class="th-front-page-summery">
					<?php echo nek9sar_excerpt(45); ?>
				</article>
				<a class="singlepage-widget-moretag" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>"><?php _e( 'Read More', 'nek9sar' ) ?></a>
...
function nek9sar_register_singlepage_widget() {
	register_widget( 'nek9sar_SinglePage_Widget' );
}
add_action( 'widgets_init', 'nek9sar_register_singlepage_widget' );


/**
 * Widget to display testimonial.
 */
class nek9sar_Testimonial_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_testimonial', // Base ID
			__( 'nek9sar Testimonial', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays a testimonial.Best for Business Template Left/Right Area.', 'nek9sar' ), ) // Args
		);	
	}
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php _e( 'Description', 'nek9sar' ); ?>:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('name'); ?>"><?php _e( 'Name', 'nek9sar' ); ?>:</label>
		<input class="widefat" id="<?php echo $this->get_field_id('name'); ?>" name="<?php echo $this->get_field_name('name'); ?>" type="text" value="<?php echo esc_attr($name); ?>" />
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'designation' ); ?>"><?php _e( 'designation', 'nek9sar' ); ?>:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'company_name' ); ?>"><?php _e( 'Company Name', 'nek9sar' ); ?>:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'company_url' ); ?>"><?php _e( 'Company Url', 'nek9sar' ); ?>:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php _e('Image', 'nek9sar') ?></label>
	</p>
	<p><?php _e( 'Upload a square image( width : height = 1 : 1). Do not upload large images, that will increase the site loading time.', 'nek9sar' ) ?></p>
	<div class="media-picker-wrap">
        ...
        <a class="media-picker-button button" onclick="mediaPicker(this.id)" id="<?php echo $this->get_field_id( 'image' ).'mpick'; ?>"><?php _e('Select Image', 'nek9sar') ?></a>
    </div>	
...
function nek9sar_register_testimonial_widget() {
	register_widget( 'nek9sar_Testimonial_Widget' );
}
add_action( 'widgets_init', 'nek9sar_register_testimonial_widget' );
 ...
 */
class nek9sar_Featured_Images extends WP_Widget {

	function __construct() {
		parent::__construct(
			'nek9sar_featured_images', // Base ID
			__( 'nek9sar Featured Images', 'nek9sar' ), // Name
			array( 'description' => __( 'Displays images.You can show customer logos or any other images by this widget.Best for Business Template Top/Bottom Area.', 'nek9sar' ), ) // Args
		);	
	}
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'image_url'.$i ); ?>"><?php _e('Image ', 'nek9sar'); echo $j; ?></label>
	</p>
	<div class="media-picker-wrap">
        ...
        <a class="media-picker-button button" onclick="mediaPicker(this.id)" id="<?php echo $this->get_field_id( 'image_url'.$i ).'mpick'; ?>"><?php _e('Select Image', 'nek9sar') ?></a>
    </div>

	<p>
		<label for="<?php echo $this->get_field_id( 'image_redirect_url'.$i ); ?>"><?php _e( 'Image Redirect Link ', 'nek9sar' ); echo $j; ?></label>
...
function nek9sar_featured_images() {
	register_widget( 'nek9sar_Featured_Images' );
}
add_action( 'widgets_init', 'nek9sar_featured_images' );	